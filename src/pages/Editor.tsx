  ...
  return (
    <div className="pb-32">
      ...
      <div className="px-4 pt-4">
        ...
        <h2 className="text-2xl font-bold text-ink dark:text-white">{process.title}</h2>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{process.summary}</p>
        ...
        {process.transcript && process.transcript.length > 0 && (
          <div className="mt-3 flex items-center gap-2 rounded-xl bg-mist-50 px-3 py-2 text-xs text-slate-500 dark:bg-white/5 dark:text-slate-400">
            🎙️ Transcripción original de la sesión ({process.transcript.length} líneas)
          </div>
        )}

        <div className="mt-6 flex items-center justify-between">
          <h3 className="text-lg font-bold text-ink dark:text-white">Pasos de ejecución</h3>
          <button
        ...
        <div className="mt-3 space-y-3">
          {process.steps.map((step) => (
            <div key={step.id} className="rounded-2xl border border-slate-100 bg-white p-4 shadow-card dark:border-white/10 dark:bg-ink-soft">
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className={`font-semibold ${step.done ? 'text-slate-400 line-through' : 'text-ink dark:text-white'}`}>
                    {step.title}
                  </p>
                  ...
                  {(step.priority || step.assignee) && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {step.priority && (
                        <span className="rounded-lg bg-mist-100 px-2 py-1 text-xs text-slate-600 dark:bg-white/5 dark:text-slate-300">
                  ...
                  {process.sourceType === 'manual' && (
                    <button className="mt-2 flex items-center gap-1 rounded-lg border border-dashed border-slate-200 px-3 py-1.5 text-xs text-slate-400 dark:border-white/10">
        ...
        <div className="mt-6 flex items-center justify-between">
          <h3 className="text-lg font-bold text-ink dark:text-white">Colaboradores</h3>
        </div>
        <div className="mt-3 space-y-2">
          {process.contributors.map((c) => (
            <div key={c.id} className="flex items-center justify-between rounded-2xl border border-slate-100 bg-white p-3 shadow-card dark:border-white/10 dark:bg-ink-soft">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-ink dark:text-white">{c.name}</p>
                  <p className="text-xs text-slate-400">{c.role}</p>
                </div>
              </div>
            </div>
          ))}
          <button className="flex w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-200 py-3 text-sm font-medium text-slate-500 dark:border-white/10 dark:text-slate-400">
            <UserPlus size={16} /> Invitar colaboradores
          </button>
        </div>

        <div className="mt-6 flex gap-3">
          <div className="flex-1 rounded-2xl border border-slate-100 bg-white p-4 shadow-card dark:border-white/10 dark:bg-ink-soft">
            <p className="text-[11px] uppercase text-slate-400">Tiempo de ejecución</p>
            <p className="text-lg font-bold text-ink dark:text-white">{process.timeToExecute ?? '—'}</p>
          </div>
          <div className="flex-1 rounded-2xl border border-slate-100 bg-white p-4 shadow-card dark:border-white/10 dark:bg-ink-soft">
